...
const selectedCity = usedCities[0]
...
describe('Card', () => {
  ...
  it('should be able to display the city information, on click, for the team being viewed', () => {
    expect(wrapper.state()).toEqual({teamInfo: true, matchedCity: null })
    wrapper.find('.card-btn').simulate('click')
    expect(wrapper.state()).toEqual({matchedCity: usedCities[0], teamInfo: false})
  })

  it('should be able to toggle from team view to city view upon click', () => {
    wrapper.find('.card-btn').simulate('click')
    expect(wrapper.state('teamInfo')).toEqual(false)
    wrapper.find('.card-btn').simulate('click')
    expect(wrapper.state('teamInfo')).toEqual(true)
  })
})